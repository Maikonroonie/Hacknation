...
const Header = () => {
  ...
  // Funkcja, która podświetla aktywny link na czerwono
  const linkClass = (path: string) => 
    `text-sm font-medium px-3 py-2 rounded-md transition-colors ${
      location.pathname === path 
        ? 'text-pko-red font-bold bg-gray-50' 
        : 'text-pko-text hover:text-pko-navy hover:bg-gray-50'
    }`;
  ...
  return (
    <header className="sticky top-0 z-50 bg-white shadow-md border-b-2 border-pko-red">
      <div className="container mx-auto px-4 h-16 flex items-center justify-between relative">
        ...
        <Link to="/" className="flex items-center gap-3 group text-decoration-none z-10">
          <div className="w-10 h-10 bg-pko-navy text-white flex items-center justify-center font-bold text-xl rounded-sm group-hover:bg-pko-red transition-colors">
            PKO
          </div>
          <div className="flex flex-col">
            <span className="text-pko-navy font-bold text-lg leading-tight">Indeks Branż</span>
            <span className="text-[10px] text-gray-500 uppercase tracking-widest">Dashboard Analityczny</span>
          </div>
        </Link>
